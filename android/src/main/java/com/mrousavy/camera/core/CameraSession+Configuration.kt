...
internal fun CameraSession.configureOutputs(configuration: CameraConfiguration) {
  ...
  if (previewConfig != null) {
    ...
    val previewBuilder = Preview.Builder() // Use a distinct name for the builder
    // Configure Preview Output with previewBuilder
      ...
      previewBuilder.setTargetFrameRate(fpsRange)
      }
    ...
    val appContext = (context as ThemedReactContext).reactApplicationContext
    // Create the actual CameraCaptureSession.CaptureCallback
    val actualSessionCaptureCallback = object : CameraCaptureSession.CaptureCallback() {
      override fun onCaptureProgressed(
        ...
        // Your onCaptureProgressed logic here (if any)
      }
      override fun onCaptureCompleted(
        ...
        // Your onCaptureCompleted logic (brightness calculation)
        var brightness = 250 //default
        val exposureTimeResult = result.get(CaptureResult.SENSOR_EXPOSURE_TIME)
        val sensitivityResult = result.get(CaptureResult.SENSOR_SENSITIVITY)
        if (exposureTimeResult != null && sensitivityResult != null) {
          val t = exposureTimeResult.toDouble() * 1.0e-9
          val s = sensitivityResult.toDouble()
          if (t > 0 && s > 0) { // Avoid division by zero
            val lightLevel = (brightness.toDouble() / (t * s)).toInt()
            try {
              appContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter::class.java).emit("brightnessEvent", lightLevel)
            } catch (e: Throwable) {
              Log.e("CameraSession Light:", "Exception in brightness event: ${e.message}")
            }
          } else {
            Log.w("CameraSession Light:", "Exposure time or sensitivity is zero, cannot calculate light level.")
          }
        } else {
          Log.w("CameraSession Light:", "Exposure time or sensitivity is null.")
        }
      }
    }
    previewExtender.setSessionCaptureCallback(actualSessionCaptureCallback) // Pass the actual callback
    val builtPreview = previewBuilder.build() // Build the Preview object
    builtPreview.setSurfaceProvider(previewConfig.config.surfaceProvider)
    previewOutput = builtPreview // Assign to the class member previewOutput
  } else {
    previewOutput = null
  }
