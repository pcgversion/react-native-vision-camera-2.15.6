...
class CameraSession(internal val context: Context, internal val callback: Callback) :
  ...
  private fun configureOrientation() {
    ...
    // Outputs Orientation
    var lastOrientation = orientationManager.oldOuputOrientation?.toSurfaceRotation().let { rotation ->
      rotation      
    }
    orientationManager.outputOrientation.toSurfaceRotation().let { outputRotation ->
      var adjustedRotation = if (outputRotation == 2) 1 else if(outputRotation == 0) 0 else outputRotation
